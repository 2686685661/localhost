...
function createDoc(docRUL,docName,docSize) {
    ...
    if(docRUL.length>0) {
        for (var i = 0, len = docRUL.length; i < len; i++) {
            htmltext += " <li id='uploadList_"+i+"' class='jFiler-item jFiler-no-thumbnail' style='width: 49%;' data-jfiler-index='0'>" +
            ...
            htmltext +=" </div></div> <div class='jFiler-item-assets jFiler-row'>" +
                ...
                "<li><a style='"+docRUL[i]+"' class='icon-jfi-trash jFiler-item-trash-action' onclick='depic("+i+",1)'> <span style='display: none;'>"+docName[i]+"</span></a></li></ul></div></div></div> </li>";
        }
    }
    ...
    if(imgRUL.length>0) {
        for (var j = 0, len = imgRUL.length; j < len; j++) {
            htmltext += "<div id='uploadList_"+j+"' class='upload_append_list'  onmouseover='hov(this)' onmouseout='seout(this)'>" +
                ...
                " <span id='del"+j+"' class='file_del' data-index=1' title='删除' onclick='depic("+j+",0)'></span>" +
                " </div> </div>" +
...
function depic(id,type) {
    var imtinput=null;
    if(type == 0) {
        imtinput =document.getElementById("fileImage");
    }else if(type == 1) {
        imtinput = document.getElementById("demo-fileInput-4");
    }
...
function fasonpic(type) {
    ...
    for(var i =0;i < thefiles.length;i++) {
        (function (e) {
            if(thefiles[i] != undefined) {
                formdata.append(i,thefiles[i]);
            }
        })(i);
    }
